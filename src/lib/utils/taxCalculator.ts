...
export function calculatePropertyTransferTax(
  ...
  if (valueInUVT <= TAX_BRACKETS.EXEMPT_THRESHOLD) {
    ...
    bracket = "high";
    const exemptAmount = TAX_BRACKETS.EXEMPT_THRESHOLD * UVT_VALUE_2025;
    const lowBracketAmount =
      (TAX_BRACKETS.LOW_BRACKET_MAX - TAX_BRACKETS.EXEMPT_THRESHOLD) *
      UVT_VALUE_2025;
    ...
    taxAmount =
      breakdown.lowBracketTax! + breakdown.highBracketTax! + fixedAmount;
...
export function getBracketDescription(
  ...
  switch (bracket) {
    case "exempt":
      return `Exento (menos de ${formatUVT(
        TAX_BRACKETS.EXEMPT_THRESHOLD
      )} UVT)`;
    case "low":
      return `Baja (${formatUVT(TAX_BRACKETS.EXEMPT_THRESHOLD)} - ${formatUVT(
        TAX_BRACKETS.LOW_BRACKET_MAX
      )} UVT)`;
    case "high":
